...
class GestaltMatcherDataset(Dataset):
    def __init__(self,
        ...
        print(f"The target path: {target_file_path}")
        print(f"The image directory: {imgs_dir}")
    ...
    def handle_target_file(self, delim=','):
        df = pd.read_csv(self.target_file, delimiter=delim)
    ...
    def get_image_ids(self):
        return self.targets["image_id"]

    def get_image_labels(self):
        return self.targets["label"]

    def get_unique_labels(self):
        return self.targets.label.unique()
